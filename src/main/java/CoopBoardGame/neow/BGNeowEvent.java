...
public class BGNeowEvent extends AbstractEvent {

    private static final Logger logger = LogManager.getLogger(BGNeowEvent.class.getName());
    private static final CharacterStrings characterStrings =
        CardCrawlGame.languagePack.getCharacterString("Neow Event");
    ...
    public static final String text22 = CardCrawlGame.languagePack.getCharacterString(
        "Neow Reward"
    ).TEXT[22];
        ...
        if (shouldSkipNeowDialog()) {
            ...
            AbstractDungeon.topLevelEffects.add(
                new LevelTransitionTextOverlayEffect(
                    AbstractDungeon.name,
                    AbstractDungeon.levelNum,
                    true
                )
            );
        } else {
    ...
    public void update() {
        ...
        if (
            this.pickCard && //pickCard only applies to dailyBlessing
        ...
        if (this.screenNum == 100) {
            if (BGNeowQuickStart.rewardIndex == 3 || BGNeowQuickStart.rewardIndex == 10) {
                if (!AbstractDungeon.isScreenUp) {
                    ...
                    int totalCards = BGNeowQuickStart
                        .quickStartQuantities[effectiveRewardIndex][BGNeowQuickStart.actNumber - 2];
        ...
        switch (this.screenNum) {
            ...
            case 99: //leave event or choose quickskip
                ...
                switch (buttonPressed) {
                    ...
                    case 3:
                        ...
                        this.screenNum = 100;
                        talk(TEXT[10]); //Time for a CHALLENGE...
                        this.roomEventText.updateDialogOption(
                            0,
                            EXTRA[42] +
                                BGNeowQuickStart
                                    .quickStartQuantities[1][BGNeowQuickStart.actNumber - 2] +
                                EXTRA[43]
                        );
                }
                return;
            case 100:
                ...
                switch (BGNeowQuickStart.rewardIndex) {
                    case 1: //gain gold
                        clearAllRewards();
                        int goldAmount = BGNeowQuickStart
                            .quickStartQuantities[BGNeowQuickStart.rewardIndex][BGNeowQuickStart.actNumber -
                        2];
                        CardCrawlGame.sound.play("GOLD_JINGLE");
                        AbstractDungeon.player.gainGold(goldAmount);
                        BGNeowQuickStart.rewardIndex += 1;
                        upcomingRewards = BGNeowQuickStart
                            .quickStartQuantities[BGNeowQuickStart.rewardIndex][BGNeowQuickStart.actNumber -
                        2];
                        this.roomEventText.updateDialogOption(
                            0,
                            EXTRA[44] + upcomingRewards + EXTRA[45]
                        );
                        break;
                    case 2: //card rewards
                        numRewards = BGNeowQuickStart
                            .quickStartQuantities[BGNeowQuickStart.rewardIndex][BGNeowQuickStart.actNumber -
                        2];
                        if (numRewards > 0) {
                    ...
                    case 3: //transform
                        ...
                        if (upcomingRewards == 1) {
                            this.roomEventText.updateDialogOption(
                                0,
                                EXTRA[48] + upcomingRewards + EXTRA[49]
                            );
                        } else {
                            this.roomEventText.updateDialogOption(
                                0,
                                EXTRA[48] + upcomingRewards + EXTRA[50]
                            );
                        }
                        break;
                    case 4: //Roll the Die
                        switch (BGNeowQuickStart.rollTheDieReward) {
                            ...
                            case 4: //potion + 2G
                                ...
                                for (
                                    int j = 0;
                                    j < AbstractDungeon.combatRewardScreen.rewards.size();
                                    j++
                                ) {
                                    if (
                                        ((RewardItem) AbstractDungeon.combatRewardScreen.rewards.get(
                                                j
                                            )).type ==
                    ...
                    case 5: //always 1 potion
                        ...
                        this.roomEventText.updateDialogOption(
                            0,
                            EXTRA[52] + upcomingRewards + EXTRA[53]
                        );
                        break;
                    case 6: //relics
                        numRewards = BGNeowQuickStart
                        ...
                        this.roomEventText.updateDialogOption(
                            ...
                            EXTRA[54] +
                                upcomingRewards +
                                (upcomingRewards == 1 ? EXTRA[55] : EXTRA[56])
                        );
                        break;
                    case 7: //rare cards
                        talk(EXTRA[71]);
                        ...
                        this.roomEventText.updateDialogOption(
                            ...
                            EXTRA[64] +
                                upcomingRewards +
                                (upcomingRewards == 1 ? EXTRA[65] : EXTRA[66])
                        );
                        break;
                    case 8: //boss relics
                        BGNeowQuickStart.rewardCounter = 0;
                        ...
                        if (upcomingRewards > 0) {
                            ...
                            this.roomEventText.updateDialogOption(
                                0,
                                EXTRA[58] + upcomingRewards + EXTRA[59]
                            );
                        }
                        break;
                    case 9: //more cards
                    ...
                    case 10: //remove
                        ...
                        int actualRewards = 0;
                        if (buttonPressed == 0) {
                            if (numRewards == 2) actualRewards = 2;
                            else actualRewards = 1;
                        } else if (buttonPressed == 1) {
                            if (numRewards == 2) actualRewards = 1;
                            else actualRewards = 0;
    ...
    private void endlessBlight() {
        if (AbstractDungeon.player.hasBlight("DeadlyEnemies")) {
            ...
            getCurrRoom().spawnBlightAndObtain(
                Settings.WIDTH / 2.0F,
                Settings.HEIGHT / 2.0F,
                ...
                if (temp != null) {
                    ...
                    if (temp != null) {
                        ...
                        getCurrRoom().spawnBlightAndObtain(
                            Settings.WIDTH / 2.0F,
                            Settings.HEIGHT / 2.0F,
                            (AbstractBlight) new GrotesqueTrophy()
                        );
                    }
                } else {
                    getCurrRoom().spawnBlightAndObtain(
    ...
    private void dailyBlessing() {
        ...
        if (ModHelper.isModEnabled("Heirloom")) {
            getCurrRoom().spawnRelicAndObtain(
                Settings.WIDTH / 2.0F,
                Settings.HEIGHT / 2.0F,
    ...
    private void blessing(boolean previewOnly) {
        ...
        ArrayList<String> rewardoptions = new ArrayList<String>(
            Arrays.asList(
        ...
        if (previewOnly) {
            ...
            this.roomEventText.updateDialogOption(
                0,
                ((BGNeowReward) this.rewards.get(0)).optionLabel
            );
