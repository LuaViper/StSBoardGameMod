...
public class BGCenturion extends AbstractBGMonster implements DieControlledMoves, BGDamageIcons {
        ...
        switch (this.nextMove) {
            case 1:
                ...
                AbstractDungeon.actionManager.addToBottom(
                    (AbstractGameAction) new ChangeStateAction(this, "MACE_HIT")
                );
                AbstractDungeon.actionManager.addToBottom(
            ...
            case 2:
                ...
                AbstractDungeon.actionManager.addToBottom(
                    (AbstractGameAction) new GainBlockAction(
                        (AbstractCreature) (AbstractDungeon.getMonsters()).monsters.get(1),
                        this.blockAmount
                    )
                );
                break;
    ...
    private void playSfx() {
        ...
        if (roll == 0) {
            AbstractDungeon.actionManager.addToBottom(
                (AbstractGameAction) new SFXAction("VO_TANK_1A")
            );
        } else if (roll == 1) {
            AbstractDungeon.actionManager.addToBottom(
                (AbstractGameAction) new SFXAction("VO_TANK_1B")
            );
        } else {
            AbstractDungeon.actionManager.addToBottom(
                (AbstractGameAction) new SFXAction("VO_TANK_1C")
            );
        }
    }
        ...
        if (!this.isDying && !this.isEscaping) {
            ...
            AbstractDungeon.actionManager.addToBottom(
                (AbstractGameAction) new ApplyPowerAction(
                    ...
                    (AbstractCreature) this,
                    (AbstractPower) new StrengthPower((AbstractCreature) this, this.furyStr),
                    this.furyStr
                )
            );
