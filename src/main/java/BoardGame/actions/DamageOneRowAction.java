...
public class DamageOneRowAction extends AbstractGameAction {
    ...
    public void update() {
        if(target instanceof BGMultiMonster){
            this.whichRow=((BGMultiMonster)target).getCurrentRow();
        }
        ...
        if(whichRow==-1){
            if (AbstractDungeon.getCurrRoom() instanceof MonsterRoom && AbstractDungeon.lastCombatMetricKey.equals("BoardGame:Shield and Spear")) {
                ...
                monsters=AbstractDungeon.getCurrRoom().monsters.monsters;
            }
        }else{
            for(AbstractMonster m : AbstractDungeon.getCurrRoom().monsters.monsters){
                if(m instanceof BGMultiMonster){
                    if(((BGMultiMonster) m).getCurrentRow()==whichRow){
                        monsters.add(m);
                    }
                }
            }
        ...
        if (this.isDone) {
            ...
            for(int i=0;i<AbstractDungeon.getCurrRoom().monsters.monsters.size();i+=1) {
