...
public class DiscardAtEndOfTurnMultiAction extends AbstractGameAction {
    ...
    public void update() {
        if (this.duration == DURATION) {
            ...
            int j= MultiCharacter.getSubcharacters().size()-1+ MultiCharacter.handLayoutHelper.currentHand;
            for(int k = j; k>=j-(MultiCharacter.getSubcharacters().size()-1); k-=1) {
                AbstractPlayer p= MultiCharacter.getSubcharacters().get(k% MultiCharacter.getSubcharacters().size());
